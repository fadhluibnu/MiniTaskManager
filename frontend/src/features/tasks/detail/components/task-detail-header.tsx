...
const NAV_LINKS = [
  ...
  { label: 'Global Audit Trail', to: '/audit-logs' },
] as const

export default function TaskDetailHeader({ actor }: TaskDetailHeaderProps) {
  return (
    <header className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Task Detail</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          View active task information and its immutable audit history.
        </p>
      </div>
